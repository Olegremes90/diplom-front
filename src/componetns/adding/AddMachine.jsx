...
const AddMachine = ({Headers}) => {
    ...
    const[number_steerable_bridge, setNumberSteerableBridge] = useState()
    ...
    getLead()
     ...
     const onFormSubmit =(e)=>{
        ...
        axios.post('http://127.0.0.1:8000//machine/', state, {headers}).
            then((response)=>{
                console.log(response)
            setMessage('Данные были успешно добавлены!')
        }).catch((error)=>{
        ...
        <div className='container-machine-add'>
            ...
            <form action='' method='post' onSubmit={onFormSubmit}>
             ...
             <div><input placeholder="Номер трансмиссии" type='text' value={number_transmisia} onChange={e=>setNumberTransimisia(e.target.value)}/></div>
                <div><input placeholder="Номер управляемого моста" type='text' value={number_steerable_bridge} onChange={e=>setNumberSteerableBridge(e.target.value)}/></div>
                 <div><input placeholder="Договор поставки" type='text' value={contract_postavka} onChange={e=>setContractPostavka(e.target.value)}/></div>
                  <div><input placeholder="Дата отгрузки" type='text' value={date_otgruzka} onChange={e=>setDateOtgruzka(e.target.value)} onFocus={(e)=>(e.target.type = 'date')}/></div>
               ...
               <div>
                ...
                <button className='button-add-machine' type='submit'>Создать машину</button>
            </form>
        </div>
        </div>
    );
