...
const DirectoryUsel = () => {
    ...
    useEffect(()=>{
        ...
        const lead_list = await axios.get(`http://127.0.0.1:8000//usel-detail/${params.title}`, {headers}).
            then((result)=>{
    ...
    const AddUsel=(e)=>{
        ...
        axios.post(`http://127.0.0.1:8000//usel/add/`, state, {headers}).then((result)=>{
            console.log(result)
             setMessage('Данные в справочник модели узла отказа успешно добавлены!')
        }).catch((error)=>{setMessage('Упс, что-то пошло не так, повторите попытку.')})
    }
     const UpdateUsel= async(e)=>{
          e.preventDefault();
        const state = {
              'title': title_1,
            'descrip': description_1
        }
         const serv = await axios.patch(`http://127.0.0.1:8000//update-usel/${usel[0].id}`, state, {headers}).then((result)=>{
    ...
    return (
        <div>
             <div className='title-table-book'>
                  {usel?
             <h3 className='number-machine'>Cправочник модели узла отказа - {usel[0].title} </h3>
                      :<h3></h3>
                  }
              </div>
           <table className='table-book'>
             ...
             <span  className='title-forma'>Добаление в справочник узла отказа</span>
           <form  action='' method='post' onSubmit={AddUsel}>
           ...
           {usel ?
           <span  className='title-forma'>Изменить данные справочника для {usel[0].title}</span>
               :<div></div>
           }
            <form  action='' method='post' onSubmit={UpdateUsel}>
